                        ...
                        <table class="table align-items-center table-flush table-hover" id="dataTableHover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $degree->id }}</td>
                                    <td>{{ $degree->name }}</td>
                                    {{-- <td><img src="{{ asset('public/storage'.'/'.$degree->logo) }}" style="height: 80px; width:100px"></td> --}}
                                    <td>{{ $degree->created_at->diffForHumans() }}</td>
                                    <td>
                                        <button onclick="setUpdate({{ $degree->id }},'{{ $degree->name }}')" data-target="#update_modal" type="button" class="btn btn-warning"
                                            data-toggle="modal"><i class="fas fa-user-edit">Update</i></button>
                                        <button onclick="setDelete({{ $degree->id }})" data-target="#delete_modal" type="button" class="btn btn-danger"
    ...
    <div class="modal fade" id="add_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form action="{{ url('admin/degree') }}" method="POST" enctype="multipart/form-data">
                    @csrf
...
@section('scripts')
    <script>
        function setUpdate(id,name){
            $('#name_field').val(name);
            $('#update_form').attr('action', '{{ url('/') }}/admin/degree/'+id);
        }
        function setDelete(id){
            $('#delete_form').attr('action', '{{ url('/') }}/admin/degree/'+id);
        }
    </script>
@endsection